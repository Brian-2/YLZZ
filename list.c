...
{
    FILE *fp;//FILE 是一个预定义类型

    fp = fopen(filename ,"rt");//整个这个过程都是死规矩
                        //read_txt(只读打开)————wt,write_txt(只写打开)
    if (fp == NULL)
    {
        printf("Cannot open file %s.", filename);
        exit(1);//填0表示正常退出，这里填1表示不正常
    }
    value_type v;
    
    size_t cnt = 0;
    while (!feof(fp))//feof 是测试是否读到文件end的fun，是返回1,不是则返回0
    {
...
void list_save_from_file(listptr l ,const char *filename)
{
    FILE *fp;//FILE 是一个预定义类型
    
    //这里实际上没有考虑边界条件，链表为空的时候应该直接出来

    fp = fopen(filename ,"wt");//整个这个过程都是死规矩
                        //read_txt(只读打开)————wt,write_txt(只写打开)
    if (fp == NULL)
    {
        printf("Cannot open file %s.", filename);
    ...
    nodeptr p = l->head;
    while (p != NULL)//feof 是测试是否读到文件end的fun，是返回1,不是则返回0
    {
        fprintf(fp, "%d%d\n", p->data.x, p->data.y);
        p = p->next;
    }
...
int comp_x(point* v1, point* v2)
{
    return v1->x - v2->x;
}

int comp_len_to_zero(point* v1, point* v2)
{
    return (v1->x * v1->x + v1->y * v1->y) - 
    (v2->x * v2->x + v2->y * v2->y);
}

int comp_y(point* v1, point* v2)
{
    return v1->y - v2->y;
}
...
int main()
{
    ...
    list_create_from_file(&l, "data1.txt");
    list_save_from_file(&l, "data11.txt");
